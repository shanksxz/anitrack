...
const MyManga = () => {
    ...
    const [userMangaData, setUserMangaData] = useState<userMediaListResponse[]>();
    ...
    const fetchUserData = async () => {
        try {
            setLoading(true);
            const result = await userMediaList(userId,status,'UPDATED_TIME_DESC','MANGA');
            console.log(result);
            setUserMangaData(result?.Page?.mediaList);
            setLoading(false);
        } catch (error) {
            console.log(error);
    ...
    return (
        <section className='max-h-full relative p-6 text-white'>
            <h1 className='font-bold text-[2rem]'>My Manga</h1>
            ...
            <div className='mt-2'>
                ...
                </Select>
                {
                    ...
                    <div className='mt-5 max-h-[450px] flex flex-col gap-2 scrollbar scrollbar-thumb-[#7330e6] scrollbar-w-3 overflow-y-scroll'>
                        {
                            userMangaData?.map((anime : userMediaListResponse) => (
                                <MyAnimeCard
                                    key={anime.id}
                                    MyAnime={anime}
                                />
