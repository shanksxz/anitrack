...
const useUserStore = create<userState>((set) => ({
    userId:
        JSON.parse(localStorage.getItem("userDetail") || "{}").Viewer?.id || 0,
    userName:
        JSON.parse(localStorage.getItem("userDetail") || "{}").Viewer?.name ||
        "",
...
//todo : make it for production (chrome storage api)